...
const CampaignsPage = () => {
  ...
  const [selectedCampaign, setSelectedCampaign] = useState<any>(null); // Selected campaign
      ...
      try {
        ...
        setCampaigns(
          response.data.filter(
            (campaign: any) => campaign.status === "Approved"
          )
        );
      } catch (err) {
  ...
  const handleInvest = async (e: any) => {
    ...
    try {
      ...
      if (err.status === 401) {
        ...
        toast.error(err.response.data.message || "Failed to initiate investment.");
      }
    }
  };
